...
def construir_fila_resultado(
    ...
    desviacion_relativa_optimo = (
        brecha_optimo / optimo
        if brecha_optimo is not None and optimo > 0
        else None
    )
    ...
    pct_solver_total = (
        t_solver / tiempo_total
        if t_solver is not None and tiempo_total > 0
        else None
    )
    pct_cuantico_solver = (
        t_cuantico / t_solver
        if t_cuantico is not None and t_solver
        else None
    )
    pct_clasico_solver = (
        t_clasico / t_solver
        if t_clasico is not None and t_solver
        else None
    )
    ...
    fila = {
        ...
        "desviacion_relativa_optimo": desviacion_relativa_optimo,
        ...
        "t_clasico": t_clasico,
        "pct_tiempo_solver_sobre_total": pct_solver_total,
        "pct_tiempo_cuantico_sobre_solver": pct_cuantico_solver,
        "pct_tiempo_clasico_sobre_solver": pct_clasico_solver,
        ...
        "muestras_esperadas": total_muestras,
        "penalizacion": penalizacion,
        "metodo": metodo,
        "warmstart": metricas.get("warmstart", False),
        "epsilon_warmstart": metricas.get("epsilon_warmstart", None),
        "sqa_beta": metricas.get("sqa_beta"),
        "sqa_gamma": metricas.get("sqa_gamma"),
        "sqa_trotter": metricas.get("sqa_trotter"),
        "sqa_num_sweeps": metricas.get("sqa_num_sweeps"),
    }
...
def ejecutar_experimentos(casos, metodo, ruta_csv=None):
    ...
    for caso in casos:
        ...
        if metodo == "qaoa":
            ...
            # La preparacion del problema/QUBO es overhead clasico externo al solver.
            ...
            mejor_eval, metricas, historial = resolver_sqa_multi_start(
                qp,
                problema,
                caso["optimo"],
                starts=starts,
