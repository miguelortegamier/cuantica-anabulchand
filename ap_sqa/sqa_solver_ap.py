...
def crear_sampler_sqa():
    try:
        ...
        raise RuntimeError(
            "OpenJij no esta instalado. Instala la dependencia con: "
            "pip install openjij"
        ) from exc
...
def resolver_sqa_multi_start(
    ...
    for i in range(starts):
        ...
        for e in evaluaciones:
            ...
            if mejor_eval is None:
                ...
                and e["violacion"] < mejor_eval["violacion"]
            ):
                mejor_eval = e
    ...
    mejor_coste_start = [h["coste_mejor"] for h in historial_starts]
    coste_medio_start = (
        statistics.mean(mejor_coste_start) if mejor_coste_start else 0
    )
    std_coste_start = (
        statistics.pstdev(mejor_coste_start)
        if len(mejor_coste_start) > 1 else 0
    )
    tiempo_medio_start = (
    ...
    metricas = {
        ...
        "coste_medio_start": coste_medio_start,
        "std_coste_start": std_coste_start,
