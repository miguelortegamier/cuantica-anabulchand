...
def construir_registro(i, evaluado, punto_inicial, opt_val_total):
    return {
        "start": i,
        ...
        "ratio": evaluado["valor"] / opt_val_total,
    }
...
def calcular_metricas(historial, opt_val_total, t_solver, sampler):
    ...
    ratio_medio = (
        sum(h["ratio"] for h in factibles) / len(factibles)
        if factibles else 0
    )
...
def ejecutar_multi_start(
    ...
    for i in range(starts):
        ...
        evaluado = evaluar_resultado(resultado, problema)
