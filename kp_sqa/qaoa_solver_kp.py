...
def generar_registro(i, evaluado, punto_inicial, opt_val_total, warmstart):
    return {
        "start": i,
...
def calcular_metricas(historial, opt_val_total, t_solver, sampler, starts):
...
def ejecutar_multi_start_qaoa(
    ...
    for i in range(starts):
        ...
        registro = generar_registro(
            i, evaluado, punto_inicial, opt_val_total, warmstart
        )
...
def resolver_qaoa(
    ...
    metricas["shots"] = shots
    metricas["qaoa_reps"] = repeticiones
    metricas["qaoa_maxiter"] = maxiter
    metricas["qaoa_optimizer"] = "SPSA"
